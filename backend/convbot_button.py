...
BUTTON_REPLY, SECONDTRY, CONTENT2, TITLE, CONTENT, QACOIN, TIMELIMIT, SUBMIT, CANCEL, ANSWERREPLY, ANSWER, QUESTIONID, ACCOUNT_VERIFICATION, ANSWERID, CHECK, REPLYANSWER, Q2, Q3, Q4, Q5, Q6, Q7, Q8, SUREND, LOOK_FOR_REPLY, SECONDTRY_REPLY, ANSWER2, REPLY_BUTTON = range(28)
...
def reply_button(bot, update, user_data):
	...
	if user_data['a'] == 1:
	    reply_text = 'Your answer to post id '+str(question_id)+' is posted successfully.'
	    try:
		answer_id = telegramService.replyToReply(user_data, thought)
	    except Exception as e :
		print e
		reply_text = 'Your answer to post id '+question_id+' is not posted.'
		bot.send_message(chat_id=update.message.chat_id, text=reply_text)
		user_data.clear()
		return ConversationHandler.END
	else:
            reply_text = 'Your answer to question id '+str(question_id)+' is posted successfully.'
	    try:
		answer_id = telegramService.replyToPost(user_data, thought)
	    except Exception as e :
		print e
		reply_text = 'Your answer to question id '+str(question_id)+' is not posted.'
		bot.send_message(chat_id=update.message.chat_id, text=reply_text)
		user_data.clear()
		return ConversationHandler.END
        p = subprocess.Popen(["python telePushMsg.py -i '{}' -q '{}' -a '{}' -t '{}' -c '{}' -w tele".format(
	...
	if user_data['a'] == 1:
		...
		print e
		reply_text = 'Your answer to post id '+str(question_id)+' is not posted.'
		bot.send_message(chat_id=update.message.chat_id, text=reply_text)
		user_data.clear()
		return ConversationHandler.END
	else:
...
def secondtry_reply(bot, update, user_data):
	...
	update.message.reply_text('What is your edited reply?')
        return ANSWER2
...
def answer2(bot, update, user_data):
	...
	print e
	update.message.reply_text('Your answer to question id '+user_data['question_id']+' is not posted.')
	user_data.clear()
	return ConversationHandler.END
...
def check(bot, update, user_data):
    if update.message.text == 'YES':
        update.message.reply_text('What is the content of your reply?')
	user_data['a'] = 1
        return REPLYANSWER
    elif update.message.text == 'CANCEL':
        update.message.reply_text('Stop posting successfully.')
...
states={
    BUTTON_REPLY: [RegexHandler('^(TITLE|CONTENT|QACOIN|TIMELIMIT|SUBMIT|CANCEL|FIRST|SECOND)$', button_reply, pass_user_data=True), 
